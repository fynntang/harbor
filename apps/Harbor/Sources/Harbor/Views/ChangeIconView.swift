...
struct ChangeIconView: View {
  ...
  private var selectedImage: IconImage? { useBadge ? badgeImage : image }
  private var selectedTray: Data? { useBadge ? badgeTray : (useArtwork ? image?.png : letterPNG) }
    ...
    VStack(alignment: .leading, spacing: 20) {
      ...
      HStack {
        ...
        Button(store.text("应用图标")) {
          guard let image = selectedImage, let tray = selectedTray else { return }
          Task {
            if await store.changeIcon(item: item, png: image.png, trayPNG: tray) {
              if useBadge { badge.save(for: item.profile.app_bundle) }
              dismiss()
            }
          }
        }
        .keyboardShortcut(.defaultAction).buttonStyle(.borderedProminent)
        .disabled(selectedImage == nil || selectedTray == nil || store.busy)
      }
    }
